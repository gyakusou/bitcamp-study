...
public class Exam61 {
  ...
  // score[-1] = 100; runtime error! 실행 오류
  // score[5] = 100; runtime error! 실행 오류
  
  // 배열 레퍼런스와 인스턴스를 따로 선언하기

  // null 주소를 0으로 선언, 메모리를 가리키지 않음
  int[] arr1 = null;

  // arr1[0] = 100; // runtime error 실행오류
  ...
  // 배열 instance 의 각 항목은 생성되는 순간 기본값으로 자동 초기화된다.
  ...
  */
  ...
  // 그럼 arr1에 저장 되었던 기존 배열 인스턴스에는
  // 어떻게 접근하는가?
  // => 처음 생성했떤 배열 인스턴스의 주소를 갖고 있는
  // 레퍼런스가 없기 때문에 접근할 수 없다.
  // => 주소를 잃어버려 접근 할 수 없는 메모리를
  // "가비지(garbage)" 라고 부른다.
  // 가비지는 JVM 실행 중에 메모리가 부족할 때 
  // 가비지 수집기(garbage collector) 에 의해
  // 메모리에서 해제 된다.
  // 물론 JVM을 종료하면 당연히 JVM이 사용하던 모든 메모리는 os에
  // 반납되기 때문에 
  // 가비지 또한 반납될 것이다.   
