...
public class Exam0120 {
  ...
  public static void main(String[] args) {
    // => 동시에 실행 할 코드를 담고 있는 스레드 객체를 생성한다.
    // => 그리고 현재 실행과 분리하여 작업을 시작시킨다.
    // => JVM은 이 스레드에 들어있는 코드와 다음의 진행하는 코드를
    // 왔다갔다 하면서 처리할 것이다.
    //
    new MyThread().start();
...
//
// ## 멀티태스킹 (multi-tasking)
// - 한개의 CPU가 여러 코드를 동시(?)에 실행하는 것
// - 실제는 일정한 시간을 쪼개 이 코드와 저 코드를 왔다 갔다 하면서 실행한다.
// - 그럼에도 불구하고 외부에서 봤을 때는 명령어가 동시에 실행되는 것처럼 보인다.
// - 왜?
// - CPU 속도가 워낙 빠르기 때문이다.
//
// ## CPU의 실행 시간을 쪼개서 배분하는 전략 : CPU Scheduling 또는 프로세스 스케쥴링
// - CPU에 실생 시간을 쪼개 코드를 실행하는 방법이다.
//
// 1. Round-Robin 방식
// - Windows OS 에서 사용하는 방식
...
// - 문제점
// 우선 순위가 낮은 프로그램인 경우 CPU시간을 배정 받지 못하는 문제가 발생했다.
// 그래서 몇 년이 지나도록 실행 되지 않는 경우가 나타났다.
// - 해결책
...
// - 그리고 분기 된 프로세스를 실행시켜서 작업을 동시에 진행하게 한다.
//
// - 장점
// 분기하기가 쉽다. fork() 호출
// 즉 구현(프로그래밍)하기가 쉽다.
...
// 실행 위치 및 정보(conetxt) 를 저장하고 로딩하는 과정이 필요하다.
...
// - 그냥 단독적인 프로세스 처럼 동일한 실행 시간을 부여한다.
//
