...
public class Exam0210 {
  public static void main(String[] args) throws Exception {
    ...
    // => 스레드는 JVM에서 관리 하는 것이 아니라, OS가 관리한다.
    // => 즉 OS의 스레드를 이용하는 것이다.
    // => 따라서 우선 순위에 따라 실행 스케쥴을 어떻게 관리할지는 OS에 따라 다르다.
    // => Windows OS는 우선 순위를 크게 고려하지 않는다.
    // 그래서 Windows에서 실행할 때는 우선 순위에 영향을 적게 받을 것이다.
    // => Unix, Linux 계열 OS는 우선순위를 고려한다.
    // 그래서 이런 OS에서 실행할 때는 우선순위에 영향을 받을 것 이다.\
    //
    // 주의
    // => JAVA 의 캐치프레이즈가 "Write once, run anywhere" 이다.
    // => 즉 OS에 상관 없이 동일하게 동작하게 만드는 것이 자바의 목적이다.
