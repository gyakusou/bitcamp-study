...
public class Exam0113 {
  ...
  static class Box<T> {
    ...
    // 그래서 T 변수를 "타입 파라미터"라 부른다.
    // 타입 파라미터도 변수 이기 때문에 이름을 다음과 같이 지어도 된다.
    ...
    // V - value
    // N - number
    // 여러개의 파라미터를 선언해야 할 경우 보통 두번째 파라미터 부터 다음의 알파벳을 사용한다.
    // S, U, V 등
    // 예) class Box<T,S,T,V> {...}
    //
    T value;
  ...
  public static void main(String[] args) {
    ...
    //인스턴스를 생성할 때도 어떤 타입의 값을 다룰 인스턴스 인지 지정해야 한다.
    box1 = new Box<Member>();
    ...
    // 메서드를 사용할 때 그 타입 전용 클래스인 것 럼 쓰면 된다.
