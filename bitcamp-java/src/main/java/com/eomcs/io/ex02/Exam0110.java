...
public class Exam0110 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // - 예) 파일, 메모리, 네트워크 연결 등
    // - 이런 지원을 사용하는 클래스는 항상 자원을 해제시키는 close()라는 메서드가 있다.
    // - 이번 예제에서 다루는 FileOutputStream 클래스에도 close()가 있다.
    // - 따라서 FileOutputStream 객체를 사용한 후에는 close()를 호출하여
    // 사용한 자원을 해제시켜야 한다.
    // - close()를 호출하면,
    // - FileOutputStream이 작업하는 동안 버퍼(임시메모리)를 비운다.
    // - OS에서 제공한 파일과의 연결을 끊는다.
    //
    out.close();
    // 물론, JVM을 종료하면 JVM을 실행하는 동안 사용했던 모든 자원은
    // 자동으로 해제된다.
    // 이런 예제처럼 짧은 시간동안 실행되는 경우 close()를 호출하지 않아도
    // 자원을 해제시키는데 문제가 없다.
    // 문제는 24시간 365일 멈추지 않고 실행하는 서버프로그램인 경우
    // 사용한 자원을 즉시 해제시키지 않으면
    // 자원 부족 문제가 발생한다.
    //
    // 결론
    // - 간단한 코드를 작성 하더라도 close()를 호출하는 것에 게을리 하지 마라
    //
...
// - 바이너리 파일을 편집하려면 전용 해당 파일 포맷을 이용하는 전용 프로그램이 필요하다.
//
// 2) 텍스트 파일
// - 기본 텍스트 편집기(메모장, vi 에디터 등) 로 편집할 수 있는 파일을 말한다.
// - 예) .txt, .csv, .html, .js, .css, .xml, .bat, .c, .py, .php, .docx, .pptx, .xlsx 등
// - 텍스트 파일은 전용 에디터가 필요 없다.
// - 텍스트를 편집할 수 있는 에디터라면 편집 후 저장 해도 유효하다.
//
// ## 바이너리 데이터 읽고, 쓰기
// - 읽고 쓸때 중간에서 변환하는 것 없이 그대로 바이트 단위로 그대로 읽고 써야 한다.
// - InputStream/OutputStream 계열의 클래스를 사용하라
//
// ## 텍스트 데이터 읽고, 쓰기
// - 읽고 쓸때 중간에서 문자 코드표에 따라 변환하는 것이 필요하다.
