...
public class Exam0110 {
  public static void main(String[] args) throws Exception {
    ...
    //
    // [이클립스에서 실행]
    // - 출력할 때 UCS2 를 UTF-8 코드로 변환하여 출력한다.
    //
    // [콘솔창에서 실행]
    // - OS가 기본으로 사용하는 문자코드로 변환하여 출력한다.
    // - windows : MS949
    // - linux/unix/macOS : UTF-8\
    // - 만약 JVM을 실행할 때 '-Dfile.encoding=문자코드표' 옵션을 지정했다면,
    // - 해당 옵션에 지정된 문자 코드로 변환하여 출력한다.
    // ★
    // => 이 예제를 이클립스에서 실행한다면
    // 앞의 2바이트는 버리고,
    ...
    // - 앞의 2 바이트(7a6b) 는 버린다.
    // - 뒤의 2 바이트(ac00) 은 UTF-8(bac00) 코드 값으로 변환되어 출력된다.

    // UCS2에서 영어 A는 0041 이다.
    // 출력하면 , UTF-8 코드값 (41)이 출력된다.
    out.write(0x7a5f0041);
    // - 앞의 2바이트 (7a5f)는 버린다.
    // - 뒤의 2바이트는 UTF-8(41) 코드 값으로 변환되어 출력된다.
