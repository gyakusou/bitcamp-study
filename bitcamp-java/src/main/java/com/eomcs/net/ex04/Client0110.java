...
// stateful 방식
//
// => 서버와 연결한 후, 클라이언트에서 연결을 끊을 때 까지
// 계속해서 연결을 유지하며 클라이언트 요청을 처리한다.
//
// => 단점:
// - 한 번 연결하면 클라이언트가 연결을 끊을 때까지 계속 유지한다.
// - 클라이언트가 작업을 요청하지 않더라도 연결정보를 계속 유지해야 하기 때문에
// 서버 메모리를 많이 차지하고 동시에 많은 클라이언트의 요청을 처리하기 힘들다.
...
// 데이터수신 <----------------|
//            |--------x-------> 끊기
// 한번 연결하면 연결이 끊을 때까지 데이터를 주고 받는다.
...
public class Client0110 {
  public static void main(String[] args) {
    ...
    try (Socket socket = new Socket("localhost", 8888);
      ...
      // 이름이 quit이 아니라면 계속 반복한다.
      // 사용자가 입력한 것이 quit 이라면 반복문 나간다.
