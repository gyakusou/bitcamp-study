...
public class CalcServer {
  ...
  static class RequestHandler extends Thread {
    ...
    @Override
    public void run() {
      // JVM과 분리하여 별도로 실행 할 코드를 이 메서드에 둔다.
      try {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      System.out.printf("클라이언트(%s)가 연결 되었음\n", //
          remoteAddr.getAddress(), remoteAddr.getPort());

      // 연결된 클라이언트가 연결을 끊기 전 까지는
      // 대기하고 있는 다른 클라이언트의 요청을 처리 할 수 없다.
      // 이것이 쓰레드를 사용하기 전의 문제점이다.
      // 해결책
      // 클라이언트와 대화하는 부분을 스레드로 분리하여 실행하라.
      RequestHandler requestHandler = new RequestHandler(socket);
      requestHandler.start();
      // 스레드를 호출하려면 start()를 호출하라.
      // start()에 내부에서 run()을 호출 할 것이다.
