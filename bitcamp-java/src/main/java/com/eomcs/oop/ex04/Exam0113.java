...
public class Exam0113 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // String 생성자는 OS의 기본 문자 코드 표로 간주하여 변환한다.
    // 따라서 위 예제를 Linux, Unix, macOS의 CLI(Command Line Interface: 콘솔창/명령창) 에서 실행하면
    // 정상적으로 한글 문자열이 출력되지만,
    // Windows에서 실행하면 한글 문자열이 깨진다.
    // 이유
    // Linux, Unix, macOS 기본으로 사용하는 문자 코드 표는 UTF-8이다.
    // Windows가 기본으로 사용하는 문자 코드표는 MS949이다.
    //
    // 그런데 이클립스에서 실행하면 깨지지 않는 이유
    // 이클립스에서 JVM을 실행할때 입 출력 기본 인코딩을 UTF-8로 설정한다.
    // 
    ...
    // PowerShell에서는 -Dfile.encoding 옵션을 제대로 처리하지 못한다.
    // 도트(.)를 분리 문자로 인식한다.
