...
public class Exam0130 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // 즉 new 명령을 통해 바로 인스턴스를 생성할 수 있다.
    // 대신 클래스 메서드를 통해 생성하도록 유도하고 있다.
    
    Calendar c1 = Calendar.getInstance(); // factory method 패턴
    Calendar c2 = Calendar.getInstance();
    System.out.println("c1 == c2");
    // getInstance () 호출 시점의 시각이 다르기 때문에 두 객체의 주소는 다르다.

    // 이렇게 자바에서 생성자의 사용 권한을 막고 클래스 메서드를 호출하여 객체를 생성하도록 유도하는 경우는 다음과 같다.
    // 1) 같은 값을 갖는 객체를 쓸데없이 여러개 생성하지 못하도록 하고 싶을때
    // => 메모리를 절약 할수 있다.
    // => 이런 방법을 사용하는 설계기법 : singleton
    // 2) 객체 생성과정이 복잡할 때,
    // => 객체 생성 코드를 단순하게 만들 수 있다.
    // => 생성된 객체를 재 활용 할수 있다.
    // => 이런 방법을 사용하는 설계기법 : Factory Method
