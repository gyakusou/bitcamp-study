...
// 작업:
// 1) worker 객체 사용 규칙을 정의한다.
//  => Worker 인터페이스 정의
// 2) 클래스를 정의할 때 worker 규칙에 따라 만든다.
//  => BlueWorker, WhiteWorker, JubuWorker 클래스 변경
// 3) worker를 사용하는 측 에서는 Worker 인터페이스에 정의된 대로 메서드를 호출한다.
// => Exam01 변경
...
public class Exam01 {
  public static void main(String[] args) {
    ...
    // 사용하려는 객체가 같은 인터페이스를 구현(같은 사용규칙에 따라 작성)하였다면,
