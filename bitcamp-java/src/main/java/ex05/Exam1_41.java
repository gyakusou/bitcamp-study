...
public class Exam1_41 {
  public static void main(String[] args) {
    ...
    float r1 = f1 + f2; 
    // f1과 f2에 들어 있는 값이 유효 자릿수라 하더라도 
    // 연산결과가 유효자릿수가 아니라면 값은 깨진다.
    // float 과 float의 연산 결과는 float이기 때문이다.
    ...
    // 기대한 결과가 나오지 않은 이유?
    // float과 float의 연산 결과는 float이다.
    // double 변수에 저장 하기 전에 이미 float 값이 되면서 일부 값이 짤렸다. = 왜곡되었다.

    // 그런데 r1 변수와 달리 뒤에 이상한 숫자가 많이 붙는 이유는 무엇인가?
    // IEEE 754의 이진수 변환 문제때문이다.
    // 4바이트 float 부동 소수점을 8바이트 double 부동 소수점 변수에 저장할 때
    // 왜곡된 값이 들어 갈 수 있다.
    // float 을 double 변수에 넣을 때 왜곡이 발생하기 때문에
    ...
    // 그럼에도 실제 출력해 보면 맨 뒤에 극한의 작은 수가 붙는다.
    //IEEE 754 2진수 변환 문제이다. 고민하지 말라
    // 어떻게 처리 할 것인가.
    // 맨 뒤에 붙는 극한의 작은 수는 그냥 잘라 버린다.
  }
}
