...
public class AuthController {
  ...
  public String facebookLogin( //
    ...
    // 엑세스 토큰을 가지고페이스북 서버에 사용자 정보를 요청한다.
    // 1. Facebook Graph API 실행하기.
    // => JSON 또는 XML을 리턴하는 HTTP 요청을 코딩 하는 방법?
    ...
    // 2. 서버에 요청하기.
    // => 첫번째 파라미터 : 요청 URL
    // => 두번째 파라미터 : 서버가 응답한 JSON을 어떤 타입의 개체로 만들지 지정
    Map<String, Object> response = restTemplate.getForObject( //
        "https://graph.facebook.com/v7.0/me?access_token={1}&fields={2}", Map.class, accessToken,
        "id,name,email");
    ...
    if (email == null) {
      logger.info("페이스북 엑세스 토큰이 무효하다.");

      // 엑세스토큰이 무효하다면, 다시 로그인 입력 폼으로 보낸다.
      session.invalidate();
      model.addAttribute("refreshUrl", "2;url=form");
      return "auth/login";
    }
