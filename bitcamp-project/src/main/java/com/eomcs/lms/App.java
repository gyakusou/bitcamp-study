...
public class App {
  ...
  private static void loadLessonData() {
    ...
    File file = new File("./lesson.ser"); // 30

    try (ObjectInputStream in = new ObjectInputStream( // 30
        new BufferedInputStream(new FileInputStream(file)))) {
      int size = in.readInt();
      for (int i = 0; i < size; i++) {
        lessonList.add((Lesson) in.readObject()); // 30
      }
  ...
  private static void saveLessonData() {
    // 데이터가 보관된 파일을 정보를 준비한다.
    File file = new File("./lesson.ser"); // v30

    try (ObjectOutputStream out = new ObjectOutputStream( // v30
