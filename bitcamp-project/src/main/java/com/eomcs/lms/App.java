...
public class App {
  ...
  static ArrayList<Lesson> lessonList = new ArrayList<>(); // 28
  ...
  private static void loadLessonData() { // 28
    // 데이터가 보관 된 파일을 정보를 준비한다.
    File file = new File("./lesson.csv");
    Scanner dataScan = null;
    FileReader in = null;

    try {
      // 파일을 읽을 때 사용하는 도구를 준비한다.
      in = new FileReader(file);

      // .csv 파일에서 한 줄 단위로 문자열을 읽는 기능이 필요한데
      // FileReader에서 그런 기능이 없다.
      // 그래서 FileReader를 그대로 사용할 수 없고
      // 이 객체에 다른 도구를 연결하여 사용할 것 이다.
      //
      ...
      System.out.println("파일 읽기 중 오류발생!" + e.getMessage());
