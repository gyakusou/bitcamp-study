...
public class BoardHandler {
  ...
  // > 개별적으로 관리 되어야 하는 값일 경우 인스턴스 필드로 선언한다.
  Board[] boards;
  int boardCount = 0;
  ...
  // 또한 각 게시판 마다 입력 받는 방식을 다르게 할 수 있도록 인스턴스 변수로 선언하자
  Scanner input;
  ...
  // => new 명령을 실행 할 때, 호출할 생성자를 지정할 수 있다.
  // => 주로 의존 객체를 초기화 시키는 코드를 넣는다.
  // => 생성자는 리턴 값이 없고, 클래스 이름과 같은 이름으로 메서드를 정의한다.
  // => 생성자를 실행 할 때 사용할 값은 파라미터로 받는다.
  public BoardHandler(Scanner input) {
    // BoardHandler를 실행하려면 데이터를 입력 받는 도구가 반드시 있어야 한다.
    // 이런 도구를 "의존객체 dependency object"라고 한다.
    // 보통 "dependency"라고 줄여서 부른다.
    // 생성자에서 해야할 일은 인스턴스를 생성할 때
    // 이런 의존 객체를 반드시 초기화 시키도록 하는것이다.
    this.input = input;
  ...
  // 인스턴스 메서드
  // > 인스턴스 가 있어야만 호출할 수 있는 메서드 이다.
  // > 인스턴스를 사용하는 메더스인 경우 인스턴스 메서드로 선언하라
  // > 호출할 때는 반드시 인스턴스 주소를 줘야 한다
  //   인스턴스 주소.메서드명();
  // > 이렇게 인스턴스 변수 값을 다루는 메서드를 " 연산자" 라 부른다.
  //   "연산자"operation 라 부를수 있다.

  public void listBoard() { // () 주소넣기 설계도에 따라 만든 board handler의 인스턴스 주소
  ...
  public void addBoard() { // () 주소넣기 설계도에 따라 만든 board handler의 인스턴스 주소
