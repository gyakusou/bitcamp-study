...
//    좀 더 다양한 하위 클래스를 정의 하고 다룰 수 있다.
// => 추상 메서드가 대부분인 경우 인터페이스로 정의하는 것이 낫다.
//    왜? 
//    추상 클래스의 목적이 하위 클래스에게 공통 필드나 메서드를 상속해 주는 것인데,
//    대부분의 메서드가 추상 메서드라서 서브 클래스 입장에서 딱히 도움이 안돼는 상황일 때,
//    그냥 메서드 사용 규칙만 정의하는 것이 코드 관리에 낫기 때문이다.


// 추상 메서드를 인터페이스로 분리 했으면
// 다음과 같이 해당 인터페이스 규칙을 추가 하라!
// 의미:
// => AbstaractList를 상속받는 서브 클랫스는
//    반드시 AbstaractList에 추가된 규칙(List)에 따라 메서드를 정의해야 한다.
//    즉, List에 선언된 모든 추상 메서드를 반드시 구현(정의)해야 한다.
// => 구현(implement)?
//    메서드의 몸체(body)를 정의하는 것.
//    즉 메서드의 코드를 정의하는 것.
//    메서드가 해야 할 일이 무엇인지 코드를 정의하는 것.
//    예1) abstract void m1(); <--- 추상메서드
//    예2) void m1() {...} <--- 메서드 구현
//
