...
public class App {
  ...
  public static void main(String[] args) {
    ...
    // 만든 클래스가 List이다.
    // List 클래스는 실제 작업을 하는 클래스가 아니다.
    // 그럼에도 불구하고 개발자가 다음과 같이 List 객체를 사용하려 한다면 막을 수 없다.
    // => BoardHandler 의 경우 아무런 작업을 수행하지 않을 것이다.
    ...
    //
    // List 클래스(AbstractList로 이름 변경함) 를 추상 클래스로 만들면 다음과 같이 인스턴스를 생성할 수 없다.
    // 아예 인스턴스 생성을 원천적으로 차단하는 효과가 있다.
    //
    // AbstractList<Board> list = new AbstractList<>(); // 컴파일 오류
    //
